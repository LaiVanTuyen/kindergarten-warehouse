...
- [ ] Viết lại mọi truy vấn `isActive` của develop theo `visibility` — grep
      `isActive` toàn repo sau merge, kỳ vọng **0 kết quả** trong `main/java`
- [ ] Thêm matcher `GET /api/v1/comments` permitAll (§1.4)
- [ ] Kiểm FE có gọi `/categories/{...}` không; nếu có thì liệt kê thêm (§1.4)
- [ ] Favorites: đổi path theo contract §4 **và** bọc `portalVisibleTo`
- [ ] Mang `buildDownloadFileName` vào luồng download của feature
- [ ] Test mới: Guest `GET /comments` → 200; favorites không trả tài nguyên đã
      chuyển `PRIVATE`; tên file tải về với tiêu đề chứa `/` và `"`
- [ ] `mvn test` (kỳ vọng 124+) và `mvn verify -Pintegration-test` (19+)
